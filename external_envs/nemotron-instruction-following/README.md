...
| `structured_v2`, `structured_v2_direct`, `structured_v2_diversified`, `structured_v2_tool_calling` | `nvidia/Nemotron-RL-Instruction-Following-Structured-Outputs-v2` | JSON/YAML/TOML/CSV/XML parser plus schema checks where applicable |
...
Task rubrics emit subtask metrics such as `ifeval_score`, `ifeval_strict`, `schema_valid`, `calendar_judge`, and criteria pass rates. The guard wrapper adds `anti_hacking_*` metrics for missing reasoning, zero-visible-output, unclosed reasoning tags, coherency, meta-commentary, `anti_hacking_reasoning_words`, `anti_hacking_reasoning_quality`, `anti_hacking_format_reward`, `task_reward`, and the final guard multiplier.
