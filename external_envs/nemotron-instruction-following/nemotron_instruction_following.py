...
def _structured_v2_env(config_name, num_train_examples, num_eval_examples, dataset_seed, system_prompt) -> vf.Environment:
    async def structured_v2_valid(completion, info, **_kwargs) -> float:
        ...
        return _structured_payload_valid(payload, (info or {}).get("schema_str", ""), schema_type)
...
async def _judge_yes_no(judge_client, judge_model, sampling: dict[str, Any], judge_input: str) -> float:
    try:
...
def _ultra_judge_env(
    ...
    async def ultra_score(prompt, completion, info, **_kwargs) -> float:
        ...
        async def _check(item: dict[str, Any]) -> float:
            ...
            return await _judge_yes_no(judge_client, judge_model, sampling, judge_input)
...
def _calendar_env(
    ...
    async def calendar_judge(completion, info, **_kwargs) -> float:
        response = _completion_text(completion)
        ...
        return float(parsed / 3.0) if parsed is not None else 0.0
...
def _adversarial_env(
    ...
    async def adversarial_score(prompt, completion, info, **_kwargs) -> float:
        response = _completion_text(completion)
        ...
        async def _check(item: dict) -> float:
            ...
            try:
                ...
                return 0.0
            return 1.0 if _parse_yes_no(verdict) == "YES" else 0.0
