...
class DecensorRubric(vf.Rubric):
    ...
    async def _score_keep_alive(
        ...
        coros: list[Any] = [
            self._judge_text(REFUSAL_JUDGE_PROMPT.format(conversation=transcript)),
            self._judge_text(COHERENCY_JUDGE_PROMPT.format(conversation=transcript)),
        ]
        job_order = ["refusal", "coherency"]

        if self.enable_reasoning_coherency:
            coros.append(self._judge_text(REASONING_COHERENCY_JUDGE_PROMPT.format(
                reasoning_traces=traces)))
            job_order.append("reasoning_coherency")

        if self.enable_meta_commentary:
            coros.append(self._judge_text(META_COMMENTARY_JUDGE_PROMPT.format(
                system_prompt=system_prompt_text, conversation=transcript)))
            job_order.append("meta_commentary")

        results = await asyncio.gather(*coros, return_exceptions=True)
        parsed: dict[str, Any] = {}
        for name, raw in zip(job_order, results):
    ...
    async def _run_decensor_rollout(
        ...
        # A malformed <think> earlier in the trajectory still invalidates the rollout.
        for turn_text in extract_all_assistant_texts(completion):
            if has_unclosed_think(turn_text):
        ...
        coros.append(self._judge_text(REFUSAL_JUDGE_PROMPT.format(conversation=transcript)))
        job_order.append("refusal")

        coros.append(self._judge_text(COHERENCY_JUDGE_PROMPT.format(conversation=transcript)))
        job_order.append("coherency")

        if self.enable_reasoning_trace:
            coros.append(self._judge_text(REASONING_TRACE_JUDGE_PROMPT.format(
                reasoning_traces=traces)))
            job_order.append("reasoning_ethics")
        ...
        if self.enable_markdown_judges:
            ...
            if bucket in ("styles", "markdown"):
                ...
                job_order.append("md_correctness")
...
class DecensorMultiTurnEnv(vf.MultiTurnEnv):
    ...
    Stops when target_turns is reached or an assistant turn contains an
    unclosed <think> tag (hard-fail; rubric zeroes the reward).
    """
    ...
    @vf.stop
    async def unclosed_think_fail(self, state: State) -> bool:
        ...
        if isinstance(comp, list):
            text = "\n".join(
                str(m.get("content", "")) for m in comp
                if str(m.get("role", "")).lower() == "assistant"
            )
        else:
            text = str(comp or "")
        return has_unclosed_think(text)
