...
Workplace and indirect-prompt-injection are exposed as explicit proxy
selectors only; they are not part of ``dataset="all"`` because the upstream
NeMo Gym versions are tool-session environments.
"""
...
from nemotron_tool_calling_guardrails import AntiHackingConfig, compose_system_prompt, guard_env, merge_system_prompt, parse_bool
...
def _input_to_prompt(payload: Any) -> list[dict[str, str]]:
...
def _prepare_prompt(payload: Any, system_prompt: str | None) -> tuple[list[dict[str, str]], list[dict[str, Any]]]:
...
class DatasetToolDefsSingleTurnEnv(vf.SingleTurnEnv):
    def __init__(self, *args, enable_native_tool_calls: bool = False, **kwargs):
...
def _build_tool_use(
    ...
    rows: list[dict[str, Any]] = []
    for row in raw:
        prompt, tool_defs = _prepare_prompt(row.get("responses_create_params"), system_prompt)
...
def _build_function_calling(filename: str, num_examples: int, seed: int, system_prompt: str | None) -> Dataset:
...
def _build_workplace(filename: str, num_examples: int, seed: int, system_prompt: str | None) -> Dataset:
...
def _build_indirect_prompt_injection(filename: str, num_examples: int, seed: int, system_prompt: str | None) -> Dataset:
...
def _build_swe_pivot(filename: str, num_examples: int, seed: int, system_prompt: str | None) -> Dataset:
...
def _tool_use_env(
    judge_client, judge_model, judge_sampling_args, num_train_examples, num_eval_examples, dataset_seed, system_prompt,
    enable_native_tool_calls=False,
) -> vf.Environment:
    def _train() -> Dataset:
        return _build_tool_use("train.jsonl", num_train_examples, dataset_seed, system_prompt)

    def _eval() -> Dataset:
        return _build_tool_use("train.jsonl", num_eval_examples, dataset_seed + 1, system_prompt)

    rubric = _make_action_rubric()
    return DatasetToolDefsSingleTurnEnv(
...
def _tool_use_pivot_env(
    ...
    def _train() -> Dataset:
        return _build_tool_use(
            "train.jsonl", num_train_examples, dataset_seed, system_prompt, TOOL_USE_PIVOT_DATASET, "tool_use_pivot"
        )

    def _eval() -> Dataset:
        return _build_tool_use(
            "train.jsonl", num_eval_examples, dataset_seed + 1, system_prompt, TOOL_USE_PIVOT_DATASET, "tool_use_pivot"
        )
...
def _function_calling_env(
    ...
    def _train() -> Dataset:
        return _build_function_calling("train.jsonl", num_train_examples, dataset_seed, system_prompt)

    def _eval() -> Dataset:
        return _build_function_calling("train.jsonl", num_eval_examples, dataset_seed + 1, system_prompt)
...
def _workplace_env(
    _judge_client, _judge_model, _judge_sampling_args, num_train_examples, num_eval_examples, dataset_seed, system_prompt,
    enable_native_tool_calls=False,
) -> vf.Environment:
    def _train() -> Dataset:
        return _build_workplace("train.jsonl", num_train_examples, dataset_seed, system_prompt)

    def _eval() -> Dataset:
        return _build_workplace("validation.jsonl", num_eval_examples, dataset_seed + 1, system_prompt)
...
def _indirect_prompt_injection_env(
    ...
    def _train() -> Dataset:
        return _build_indirect_prompt_injection("train.jsonl", num_train_examples, dataset_seed, system_prompt)

    def _eval() -> Dataset:
        return _build_indirect_prompt_injection("train.jsonl", num_eval_examples, dataset_seed + 1, system_prompt)
...
def _swe_pivot_env(
    ...
    def _train() -> Dataset:
        return _build_swe_pivot("train.jsonl", num_train_examples, dataset_seed, system_prompt)

    def _eval() -> Dataset:
        return _build_swe_pivot("train.jsonl", num_eval_examples, dataset_seed + 1, system_prompt)
...
def load_environment(
    ...
    enable_native_tool_calls: bool = False,
    **kwargs: Any,
) -> vf.Environment:
    """Tool-calling env.
